...
def add_order(order):
    """ This function takes a dictionary of order items and adds the item id,
    whether or not it was a recommended item, and if so, what item it was
    recommended for to the database
  ...
  """
    ...
    for id in order.keys():
        cur.execute("INSERT INTO rec_orders (item_id, was_rec, rec_from) \
        VALUES (%s,%s,%s)", (id, order[id]['was_rec'], order[id]['rec_from']))
